...
if ((!is_array($cats_tax) && ($cats_tax == wpqa_knowledgebase_type || $cats_tax == wpqa_knowledgebase_categories)) || (is_array($cats_tax) && (in_array(wpqa_knowledgebase_type,$cats_tax) || in_array(wpqa_knowledgebase_categories,$cats_tax)))) {
	$tax_knowledgebase_available = true;
}
$follow_text = true;
$term_list .= '<div class="col '.$tax_col.'">';
	if (isset($tax_question_available) && ($cat_style == "with_icon" || $cat_style == "icon_color" || $cat_style == "with_icon_1" || $cat_style == "with_icon_2" || $cat_style == "with_icon_3" || $cat_style == "with_icon_4" || $cat_style == "with_cover_1" || $cat_style == "with_cover_2" || $cat_style == "with_cover_3" || $cat_style == "with_cover_4" || $cat_style == "with_cover_5" || $cat_style == "with_cover_6")) {
	$questions = (has_wpqa()?(int)wpqa_count_posts_by_category(wpqa_questions_type,wpqa_question_categories,$tax_id):0);
		...
		$term_list .= (isset($tax_question_available) && $cat_style == "simple_follow" && $follow_category == "on"?"<div class='cat-sections-follow'>":"").'
		...
		if (isset($tax_question_available) && $cat_style == "simple_follow" && $follow_category == "on") {
			$cats_follwers = (int)(is_array($cat_follow)?count($cat_follow):0);
